...
* Configure =Origami=
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package origami
      ...
      :config
      (define-key evil-normal-state-map (kbd "zk") 'origami-backward-fold-same-level)
      (define-key evil-normal-state-map (kbd "zj") 'origami-forward-fold-same-level)
...
** Visit todo list
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (evil-leader/set-key "oi" 'visit-todo-list)
   #+END_SRC
